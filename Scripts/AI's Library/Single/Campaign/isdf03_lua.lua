...
-- Mission Name
local m_MissionName = _BZCCDatabase.Missions.ISDF03;
...
Functions[6] = function()
    ...
    if (not Mission.m_FixHaulerHealth) then
        if (GetDistance(Mission.m_Hauler1, "shab_check1") < 30) then
            -- Make it killable.
            SetMaxHealth(Mission.m_Hauler1, 1500);
...
function ScionBrain()
    ...
    if (IsAlive(Mission.m_Scion3)) then
        if (GetCurrentCommand(Mission.m_Scion3) == _BZCCDatabase.AICommands.CMD_NONE) then
    ...
    if (IsAlive(Mission.m_Scion5)) then
        ...
        if (not Mission.m_Scion5Switch and GetCurrentCommand(Mission.m_Scion5) == _BZCCDatabase.AICommands.CMD_NONE) then
